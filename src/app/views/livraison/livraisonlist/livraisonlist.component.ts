    ...
    cachetImg.onload = () => {
      ...
      const genererBon = (startX: number, startY: number) => {
        ...
        pdf.setFont("helvetica", "normal");
         pdf.text("N° de bon : " + element.reference, startX + 5, startY + 30);
...
searchBons(): void {
  ...
  this.filteredCommandes = this.commandes.filter(com =>
    (com.reference?.toLowerCase().includes(term)) ||
    (com.client_nom?.toLowerCase().includes(term)) ||
    (com.client_prenom?.toLowerCase().includes(term)) ||
    (com.chauffeur_nom?.toLowerCase().includes(term)) ||
    (com.chauffeur_prenom?.toLowerCase().includes(term)) ||
    (com.plaque_immatriculation?.toLowerCase().includes(term))
  );
